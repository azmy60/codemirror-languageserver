...
import { eventsFromChangeSet, formatContents, isEmptyDocumentation, offsetToPos, posToOffset, posToOffsetOrZero, prefixMatch, showErrorMessage, } from "./utils.js";
...
export class LanguageServerClient {
    ...
    clientCapabilities;
    plugins;
    constructor({ rootUri, workspaceFolders, transport, autoClose, initializationOptions, capabilities, timeout = TIMEOUT, }) {
...
export class LanguageServerPlugin {
    ...
    featureOptions;
    onGoToDefinition;
    constructor(client, documentUri, languageId, view, featureOptions, sendIncrementalChanges = true, allowHTMLContent = false, onGoToDefinition) {
    ...
    async processDiagnostics(params) {
        ...
        const diagnostics = params.diagnostics.map(async ({ range, message, severity, code }) => {
...
export function languageServerWithClient(options) {
    ...
    const extensions = [
        ViewPlugin.define((view) => {
            plugin = new LanguageServerPlugin(lsClient, options.documentUri ?? view.state.facet(documentUri), options.languageId ?? view.state.facet(languageId), view, featuresOptions, options.sendIncrementalChanges ?? true, options.allowHTMLContent ?? false, options.onGoToDefinition);
            return plugin;
        }),
    ];
    // Add shortcuts
